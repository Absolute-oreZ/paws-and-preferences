...
export default function CatCard({ src, onSwiped, className, ...props }: CatCardProps) {
  ...
  function animateOut(dir: 1 | -1, isDoubleTap: boolean) {
    ...
    if (dir === 1) {
      el.style.boxShadow = '0 0 50px 14px rgba(236, 72, 153, 0.9)'
    } else {
      el.style.boxShadow = '0 0 40px 12px rgba(100, 116, 139, 0.8)'
    }
    ...
    setTimeout(() => {
      if (isDoubleTap) {
        ...
        el.style.transition = 'transform .25s ease-out, opacity .25s ease-out, box-shadow .25s ease-out'
  ...
  return (
    <div
      ref={ref}
      className={cn('cat-card', className)}
    ...
    >
      ...
      {hearts.map(heart => (
        <div
          key={heart.id}
          className="heart-animation pointer-events-none absolute"
          style={{
            left: heart.x,
            top: heart.y,
          }}
